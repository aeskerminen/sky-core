        ...
        return(
            <div className="flex flex-col bg-slate-300 h-full m-2 p-1 mr-1 shadow-md overflow-hidden">
                <div className=" flex items-center p-1.5 bg-white">
                    <input onChange={this.handleInputChange.bind(this)} className="placeholder:text-slate-400 block w-full mr-2 bg-white border 
                ...
                <div className="grow flex flex-col p-2 gap-3 bg-white overflow-y-scroll">
                    {this.state.notes.map((note) => <Note removeButtonClick={this.removeNote} key={note.id} id={note.id} content={note.content} name={note.name}></Note>)}
                </div>
            </div>
        )
    }
